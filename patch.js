...
const startIdx = lines.findIndex((l) => l.includes('const fellows'));
...
if (startIdx !== -1) {
    ...
    for (let i = startIdx; i < lines.length; i++) {
        const line = lines[i];
        if (line.includes('[')) depth += line.split('[').length - 1;
        if (line.includes(']')) depth -= line.split(']').length - 1;
    ...
    if (endIdx !== -1) {
        let prevItemIdx = endIdx - 1;
        while (prevItemIdx > startIdx && lines[prevItemIdx].trim() === '') prevItemIdx--;
        if (prevItemIdx > startIdx && !lines[prevItemIdx].trim().endsWith(',')) {
            lines[prevItemIdx] = lines[prevItemIdx] + ',';
        }
        lines.splice(endIdx, 0, newFellow);
        fs.writeFileSync(path, lines.join('\\n'));
        console.log('Success inserted at line ' + endIdx);
    } else {
